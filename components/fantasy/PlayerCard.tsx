...
export default function PlayerCard({ player, tier = "gold", captain = false, tripleCaptain = false, substitute = false, compact = false, isDragging = false, invalidPosition = false, onClick }: PlayerCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="pointer-events-none absolute inset-[1px] z-10 bg-[linear-gradient(120deg,transparent_8%,rgba(255,255,255,.18)_21%,transparent_34%,transparent_66%,rgba(243,202,64,.12)_82%,transparent_94%)] opacity-55 transition-opacity group-hover:opacity-90" />
        ...
        <div className="absolute left-1.5 top-1.5 z-40 rounded-md border border-[#ffe88a]/35 bg-[#07111d]/95 px-1.5 py-[2px] text-[8px] font-black leading-none tracking-[.04em] text-[#fff0a2] shadow-[0_3px_9px_rgba(0,0,0,.45),0_0_10px_rgba(243,202,64,.14)] backdrop-blur-md">{positionLabel[player.position]}</div>
        ...
        <div className="relative z-30 -mt-1 min-w-0 px-1.5 text-center [transform:translateZ(18px)]">
          <div className="rounded-md border border-white/10 bg-[#080d1a]/90 px-1 py-0.5"><strong className="block truncate text-[9px] font-black leading-tight text-white">{player.name}</strong></div>
          <div className="mt-1 grid grid-cols-2 gap-1 text-[7px] font-black leading-none"><span className="rounded border border-[#f3ca40]/12 bg-black/34 px-1 py-[3px] text-[#ffe676]">{player.price.toFixed(1)}M</span><span className="rounded border border-emerald-300/10 bg-black/34 px-1 py-[3px] text-[#00e676]">{player.points} P</span></div>
        </div>
      </div>
