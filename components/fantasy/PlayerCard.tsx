    ...
    >
      ...
      >
        ...
        <div className="fiq-card-portrait pointer-events-none relative z-20 mx-auto mt-1 h-[72px] w-[72px] overflow-hidden rounded-[44%_44%_38%_38%/38%_38%_58%_58%] [transform:translateZ(8px)]">
          {safePhoto ? <img src={safePhoto} alt={player.name} className="h-full w-full object-cover object-[50%_18%]" draggable={false} /> : <div className="fiq-player-initials" aria-hidden="true">{player.name.split(" ").map(n=>n[0]).slice(0,2).join("")}</div>}
        </div>
