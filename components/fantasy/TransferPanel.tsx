...
export default function TransferPanel({onClose,coachRequest,target,onClearTarget,players,coaches,clubs,selectedIds,selectedCoachId,availableSlots,onQuickAdd,onRemovePlayer,onClearSquad,onSelectCoach,onPlayerClick}:TransferPanelProps){
 ...
 return <aside className="fiq-transfer-panel sticky top-3 flex h-[calc(100dvh-24px)] min-h-0 min-w-0 flex-col overflow-hidden rounded-[24px] border border-[#f3ca40]/25 bg-[linear-gradient(180deg,rgba(5,17,28,.985),rgba(2,10,17,.995))] shadow-[0_28px_80px_rgba(0,0,0,.55)] backdrop-blur-2xl max-[980px]:static max-[980px]:h-[68dvh] max-[980px]:min-h-[460px]">
  <div className="fiq-transfer-controls relative shrink-0 border-b border-white/[.06] p-2.5">
   ...
   {<div className="fiq-transfer-sort mt-2 grid grid-cols-[minmax(0,1fr)_92px_108px] gap-1.5"><button type="button" disabled={!!target||showingCoaches} onClick={()=>setSmartOnly(v=>!v)} aria-pressed={smartOnly||!!target} className={`fiq-smart-transfers min-h-10 rounded-lg border-2 bg-black px-2.5 py-1.5 text-left font-black ${smartOnly?"border-emerald-300/80 bg-emerald-300/[.08] text-emerald-300":"border-[#f3ca40]/80 text-[#ffe778]"}`}><span className="block text-[10px]">{smartOnly||target?"☑":"☐"} UYGUN OYUNCU</span><small className="block text-[7px] font-semibold text-white/50">Bütçe · mevki · 3 sınırı</small></button><button type="button" onClick={onClearSquad} disabled={selectedIds.length===0} className="fiq-clear-squad min-h-10 rounded-lg border-2 border-rose-300/65 bg-[linear-gradient(180deg,#1b0a0d,#08090b)] px-2 text-[9px] font-black leading-tight text-rose-100 shadow-[0_5px_16px_rgba(0,0,0,.3)] disabled:opacity-35" aria-label="Kadroyu temizle"><span className="block text-[13px]">⌫</span>KADROYU TEMİZLE</button><select value={sort} onChange={e=>setSort(e.target.value as SortMode)} aria-label="Sıralama" className="h-10 rounded-lg border-2 border-[#f3ca40]/80 bg-black px-2 text-[10px] font-black text-[#ffe778] outline-none shadow-[0_5px_16px_rgba(0,0,0,.25)]"><option value="POINTS">PUAN</option><option value="PRICE_ASC">Fiyat: Artan</option><option value="PRICE_DESC">Fiyat: Azalan</option><option value="POPULAR">Popülerlik</option></select></div>}
   {hasActiveFilters?<button type="button" className="mt-1.5 w-full rounded-md border border-[#f3ca40]/35 bg-black/70 py-1 text-[8px] font-black text-white/60" onClick={clearFilters}>FİLTRELERİ TEMİZLE</button>:null}
  </div>

  <div aria-label="Transfer sonuçları" tabIndex={0} className="fiq-transfer-results relative min-h-0 flex-1 overflow-y-auto overscroll-contain px-2 pb-8 pt-1.5 touch-pan-y [-webkit-overflow-scrolling:touch]">{showingCoaches?filteredCoaches.map(c=><CoachRow key={c.id} coach={c} selected={c.id===selectedCoachId} onSelectCoach={onSelectCoach}/>):filteredPlayers.map(player=><TransferDraggable key={String(player.id)} player={player} selected={selectedSet.has(String(player.id))} assessment={transferAssessments.get(String(player.id))??{eligible:false,reason:null}} onQuickAdd={onQuickAdd} onRemovePlayer={onRemovePlayer} onPlayerClick={onPlayerClick}/>)}{!resultCount?<div className="grid min-h-36 place-items-center px-4 text-center text-[10px] font-bold text-white/35">{smartOnly||target?"Kadro kurallarına uyan transfer bulunamadı.":"Bu filtrelerle eşleşen kayıt bulunamadı."}</div>:null}</div>
 </aside>
}
...
function CoachRow({coach,selected,onSelectCoach}:{coach:FantasyCoach;selected:boolean;onSelectCoach:(c:FantasyCoach)=>void}){return <div className="fiq-coach-transfer-row mb-1.5 grid grid-cols-[1fr_44px] items-center gap-2 rounded-lg border border-white/10 bg-[#101b29] px-2.5 py-2"><div><strong className="block truncate text-[12px] font-black">{coach.name}</strong><small className="text-[8px] text-white/45">{coach.club}</small></div><button type="button" onClick={()=>onSelectCoach(coach)} className="h-9 w-9 rounded-full border-2 border-[#f3ca40]/70 bg-black text-[#ffe778]">{selected?"✓":"+"}</button></div>}
function TransferDraggable({player,selected,assessment,onQuickAdd,onRemovePlayer,onPlayerClick}:{player:FantasyPlayer;selected:boolean;assessment:TransferAssessment;onQuickAdd:(p:FantasyPlayer)=>void;onRemovePlayer:(p:FantasyPlayer)=>void;onPlayerClick?:(p:FantasyPlayer)=>void}){const d=useDraggable({id:`transfer:${player.id}`,disabled:selected,data:{sourceSlotId:"transfer",playerId:String(player.id),sourceType:"transfer"}});return <div ref={d.setNodeRef} data-fiq-dnd="true" data-selected={selected} className={`fiq-transfer-row mb-1.5 select-none rounded-lg border px-2.5 py-2 transition-all ${d.isDragging?"border-cyan-300/60 bg-cyan-400/10 opacity-35":selected?"border-emerald-300/70 bg-[linear-gradient(90deg,rgba(16,185,129,.14),rgba(11,30,42,.92))] shadow-[inset_3px_0_0_rgba(110,231,183,.9)]":assessment.eligible?"border-[#f3ca40]/35 bg-[linear-gradient(90deg,#0b1e2a,#08151e)]":"border-white/10 bg-[#0b1e2a]"}`}><div className="grid grid-cols-[40px_minmax(0,1fr)_54px_40px] items-center gap-2"><button ref={d.setActivatorNodeRef} type="button" disabled={selected} {...d.listeners}{...d.attributes} aria-label={`${player.name} oyuncusunu sürükle`} className={`grid h-10 w-9 place-items-center rounded-md border-2 text-[14px] ${selected?"border-emerald-300/20 bg-black text-emerald-300/45":"cursor-grab touch-none border-[#f3ca40]/65 bg-black text-[#ffe778]"}`}>{selected?"✓":"⋮⋮"}</button><button type="button" onClick={()=>selected?onRemovePlayer(player):onPlayerClick?.(player)} aria-label={selected?`${player.name} kadrodan çıkar`:`${player.name} oyuncu bilgileri`} className="fiq-transfer-player-info min-w-0 text-left"><div className="flex min-w-0 items-center gap-1"><strong className="truncate text-[13px] font-black text-white">{player.name}</strong>{selected?<span className="fiq-owned-badge shrink-0 rounded-full border border-emerald-300/35 px-1.5 py-0.5 text-[6px] font-black text-emerald-200">KADRODA</span>:null}</div><small className="block text-[8px] font-semibold text-white/50">{player.club} · {player.position}</small>{!selected?<span data-reason={assessment.reason??undefined} data-eligible={assessment.eligible} className={`fiq-eligibility-badge mt-1 inline-block rounded-full border px-1.5 py-0.5 text-[6px] font-black ${assessment.eligible?"border-emerald-300/35 bg-emerald-300/10 text-emerald-200":"border-rose-300/25 bg-rose-300/[.06] text-rose-200"}`}>{assessment.eligible?"UYGUN":assessment.reason??"UYGUN DEĞİL"}</span>:null}</button><span className="text-right"><b className="block text-[10px]">{formatFantasyPrice(player.price)}M</b><small className="text-[8px] text-[#ffe45f]">{player.points} P</small></span><button type="button" disabled={!selected&&!assessment.eligible} data-remove={selected||undefined} aria-label={`${player.name} ${selected?"kadrodan çıkar":"kadroya ekle"}`} title={selected?"Kadrodan çıkar":assessment.reason??"Kadroya ekle"} onPointerDown={e=>e.stopPropagation()} onClick={e=>{e.stopPropagation();selected?onRemovePlayer(player):onQuickAdd(player)}} className="h-9 w-9 rounded-full border-2 border-[#f3ca40]/70 bg-black text-lg font-black text-[#ffe778] disabled:border-emerald-300/20 disabled:text-emerald-300/50">{selected?"−":"+"}</button></div></div>}
