...
export default function TransferPanel({ players, coaches, clubs, selectedIds, selectedCoachId, onQuickAdd, onSelectCoach, onPlayerClick }: TransferPanelProps) {
  ...
  }, [club, players, position, query, showingCoaches]);

  const filteredCoaches = useMemo(() => {
    if (!showingCoaches) return [];
    const needle = query.toLocaleLowerCase("tr");
    return coaches.filter((coach) => `${coach.name} ${coach.club} ${coach.country}`.toLocaleLowerCase("tr").includes(needle) && (club === "ALL" || coach.club === club));
  }, [club, coaches, query, showingCoaches]);
    ...
    <aside className="sticky top-3 flex max-h-[calc(100vh-24px)] min-h-[720px] min-w-0 flex-col overflow-hidden rounded-[26px] border border-cyan-300/15 bg-[linear-gradient(180deg,rgba(5,17,28,.985),rgba(2,10,17,.995))] shadow-[0_28px_80px_rgba(0,0,0,.55),inset_0_1px_0_rgba(255,255,255,.035)] backdrop-blur-2xl max-[980px]:static max-[980px]:max-h-none max-[980px]:min-h-0">
      ...
      <div className="relative border-b border-white/[.06] p-3">
        <div className="mb-2 flex items-end justify-between px-0.5">
          <div>
            <span className="text-[7px] font-black tracking-[.18em] text-[#f3ca40]">{showingCoaches ? "TEKNİK DİREKTÖR HAVUZU" : "TRANSFER HAVUZU"}</span>
            <p className="mt-0.5 text-[8px] font-bold text-white/35">{resultCount} {showingCoaches ? "teknik direktör" : "oyuncu"} listeleniyor</p>
          </div>
          <span className={["rounded-full border px-2 py-1 text-[7px] font-black", showingCoaches ? "border-violet-300/20 bg-violet-300/[.08] text-violet-200" : "border-emerald-300/15 bg-emerald-300/[.06] text-emerald-300"].join(" ")}>{showingCoaches ? "YÖNETİM" : "CANLI KADRO"}</span>
        </div>

        <div className="relative">
          <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-lg text-white/60">⌕</span>
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={showingCoaches ? "Teknik direktör ara..." : "Oyuncu ara..."} className="h-12 w-full rounded-xl border border-white/15 bg-[#07131f]/95 pl-10 pr-10 text-[12px] font-bold text-white outline-none placeholder:text-white/28 focus:border-cyan-300/45 focus:ring-2 focus:ring-cyan-300/10" />
          {query ? <button type="button" onClick={() => setQuery("")} aria-label="Aramayı temizle" className="absolute right-2 top-1/2 grid h-8 w-8 -translate-y-1/2 place-items-center rounded-full border border-white/[.04] bg-white/[.02] text-xl font-light text-white/65">×</button> : null}
        </div>

        <div className="mt-3 grid grid-cols-6 gap-1 rounded-xl border border-white/[.05] bg-black/10 p-1">
          {(["ALL", "GK", "DEF", "MID", "FWD", "COACH"] as const).map((item) => (
            <button key={item} type="button" onClick={() => { setPosition(item); setQuery(""); }} className={["h-10 rounded-lg border text-[9px] font-black tracking-[.03em] transition", position === item ? item === "COACH" ? "border-violet-200/70 bg-[linear-gradient(180deg,#7667c8,#3d356f)] text-white shadow-[0_0_18px_rgba(139,92,246,.18)]" : "border-[#f7d555] bg-[linear-gradient(180deg,#ffe779,#d7a41b)] text-[#211600]" : "border-white/[.08] bg-[#0a1724]/90 text-white/70"].join(" ")}>{tabLabel[item]}</button>
          ))}
        </div>
      ...
      <div className="relative flex-1 overflow-y-auto overscroll-contain px-2 pb-4 pt-1.5">
        {showingCoaches ? filteredCoaches.map((coach) => <CoachRow key={coach.id} coach={coach} selected={coach.id === selectedCoachId} onSelectCoach={onSelectCoach} />) : filteredPlayers.map((player) => <TransferDraggable key={String(player.id)} player={player} selected={selectedIds.includes(player.id)} onQuickAdd={onQuickAdd} onPlayerClick={onPlayerClick} />)}
        {!resultCount ? <div className="grid min-h-36 place-items-center text-center text-[10px] font-bold text-white/30">Bu filtrelerle eşleşen {showingCoaches ? "teknik direktör" : "oyuncu"} bulunamadı.</div> : null}
      </div>
    </aside>
  );
}
...
function CoachRow({ coach, selected, onSelectCoach }: { coach: FantasyCoach; selected: boolean; onSelectCoach: (coach: FantasyCoach) => void }) {
  return (
    <div className={["mb-1.5 grid grid-cols-[minmax(0,1fr)_48px] items-center gap-2 rounded-xl border px-2.5 py-2.5 transition", selected ? "border-violet-300/35 bg-violet-300/[.09] shadow-[0_0_20px_rgba(139,92,246,.08)]" : "border-white/[.06] bg-[linear-gradient(180deg,rgba(26,25,54,.92),rgba(9,24,35,.98))] hover:border-violet-300/20"].join(" ")}>
      <div className="flex min-w-0 items-center gap-2.5">
        <div className="relative grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-xl border border-violet-200/20 bg-[linear-gradient(145deg,#302b62,#122b40)] shadow-[inset_0_1px_0_rgba(255,255,255,.08)]">
          <span className="text-[11px] font-black tracking-[.08em] text-cyan-100">TD</span>
          <div className="absolute bottom-[-7px] left-1/2 h-8 w-7 -translate-x-1/2 rounded-t-full bg-[radial-gradient(circle_at_50%_18%,#c99878_0_22%,#825b46_23%_38%,#243d5b_39%)]" />
        </div>
        <span className="min-w-0"><strong className="block truncate text-[11px] font-black text-white">{coach.name}</strong><small className="mt-0.5 block truncate text-[7px] font-bold text-white/42">{coach.club}</small><small className="block truncate text-[6.5px] font-bold text-cyan-200/45">{coach.country}</small></span>
      </div>
      <button type="button" onClick={() => onSelectCoach(coach)} className={["grid h-9 w-9 place-items-center rounded-full border text-sm font-black transition", selected ? "border-cyan-200/35 bg-cyan-300/10 text-cyan-200" : "border-violet-300/45 bg-violet-300/[.08] text-violet-200 hover:bg-violet-300/[.16]"].join(" ")} aria-label={`${coach.name} teknik direktör seç`}>{selected ? "✓" : "+"}</button>
    </div>
  );
}
