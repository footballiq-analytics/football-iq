...
export default function FootballPitch({ formationLabel, formations, slots, benchSlots, captainId, tripleCaptainId, draggingPlayer, onFormationChange, onPlayerClick, onBenchPlayerClick }: Props) {
  return (
    <section className="relative mx-auto w-full max-w-[920px] overflow-visible rounded-[30px] border border-cyan-300/15 bg-[linear-gradient(180deg,#0b1329,#080d1a)] p-2 shadow-[0_36px_110px_rgba(0,0,0,.70)] sm:p-2.5">
      ...
      <div className="pointer-events-none absolute -left-1 top-24 h-[55%] w-px bg-[linear-gradient(180deg,transparent,rgba(0,229,255,.7),transparent)] shadow-[0_0_14px_rgba(0,229,255,.5)]" />
      <div className="pointer-events-none absolute -right-1 top-24 h-[55%] w-px bg-[linear-gradient(180deg,transparent,rgba(243,202,64,.65),transparent)] shadow-[0_0_14px_rgba(243,202,64,.45)]" />
                  ...
                  {lineSlots.map((slot) => {
                    ...
                    return (
                      <Droppable
                        ...
                        renderClone={(provided, snapshot) => slot.player ? (
                          <DragClone provided={provided} snapshot={snapshot}>
                            <PlayerCard compact player={slot.player} captain={slot.player.id === captainId} tripleCaptain={slot.player.id === tripleCaptainId} isDragging invalidPosition={Boolean(slot.invalidPosition)} />
                          </DragClone>
                        ) : <span />}
                      >
                        {(provided, snapshot) => (
                          <div ref={provided.innerRef} {...provided.droppableProps} className={["relative grid h-[112px] min-h-[112px] min-w-0 place-items-center overflow-visible rounded-2xl transition duration-150", draggingPlayer && canDrop ? "border border-dashed border-emerald-200/55 bg-emerald-300/[.08]" : "", draggingPlayer && !canDrop ? "border border-dashed border-rose-300/30 bg-rose-400/[.08]" : "", snapshot.isDraggingOver ? "ring-2 ring-[#f3ca40]/80 shadow-[0_0_24px_rgba(243,202,64,.24)]" : "", slot.invalidPosition ? "bg-rose-500/10 ring-1 ring-rose-400/30" : ""].join(" ")}>
                            {draggingPlayer && canDrop ? <GhostCard player={draggingPlayer} active={snapshot.isDraggingOver} /> : null}
                            {slot.player ? (
                              <div className="absolute inset-0 grid place-items-center">
                                <Draggable draggableId={`player:${slot.player.id}`} index={0}>
                                  {(dragProvided, dragSnapshot) => (
                                    <DragFrame provided={dragProvided} snapshot={dragSnapshot}>
                                      <PlayerCard compact player={slot.player!} captain={slot.player!.id === captainId} tripleCaptain={slot.player!.id === tripleCaptainId} invalidPosition={Boolean(slot.invalidPosition)} onClick={() => onPlayerClick?.(slot.player!)} />
                                    </DragFrame>
                                  )}
                                </Draggable>
                              </div>
                            ) : !draggingPlayer ? <EmptySlot position={position} /> : null}
                            <div className="pointer-events-none absolute inset-0 overflow-hidden opacity-0">{provided.placeholder}</div>
                          </div>
                        )}
                      </Droppable>
                    );
      ...
      <div className="relative mt-2 overflow-hidden rounded-[26px] border border-white/18 bg-[linear-gradient(180deg,rgba(12,24,34,.97),rgba(7,15,21,.98)_48%,rgba(10,36,18,.99))] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,.18),0_22px_48px_rgba(0,0,0,.48)] backdrop-blur-xl">
        ...
        <div className="pointer-events-none absolute inset-x-[4%] top-[42%] h-px bg-[linear-gradient(90deg,transparent,rgba(255,255,255,.35),transparent)]" />
          ...
          {benchSlots.map((slot, index) => {
            ...
            return (
              <Droppable
                key={slot.id}
                droppableId={slot.id}
                isDropDisabled={!valid}
                renderClone={(provided, snapshot) => slot.player ? (
                  <DragClone provided={provided} snapshot={snapshot}>
                    <PlayerCard compact player={slot.player} substitute tier="gold" isDragging />
                  </DragClone>
                ) : <span />}
              >
                {(provided, snapshot) => (
                  <div ref={provided.innerRef} {...provided.droppableProps} className={["relative grid h-[116px] min-w-0 place-items-center overflow-visible rounded-xl border border-white/[.07] bg-[#080d1a]/16 transition duration-150 backdrop-blur-sm", draggingPlayer && valid ? "border-emerald-300/45 bg-emerald-300/[.07]" : "", draggingPlayer && !valid ? "border-rose-300/30 bg-rose-400/[.06]" : "", snapshot.isDraggingOver ? "ring-2 ring-[#f3ca40]/75 shadow-[0_0_24px_rgba(243,202,64,.18)]" : ""].join(" ")}>
                    {draggingPlayer && valid ? <GhostCard player={draggingPlayer} substitute active={snapshot.isDraggingOver} /> : null}
                    {slot.player ? (
                      <div className="absolute inset-0 grid place-items-center">
                        <Draggable draggableId={`player:${slot.player.id}`} index={0}>
                          {(dragProvided, dragSnapshot) => (
                            <DragFrame provided={dragProvided} snapshot={dragSnapshot}>
                              <PlayerCard compact player={slot.player!} substitute tier="gold" onClick={() => onBenchPlayerClick?.(slot.player!)} />
                            </DragFrame>
                          )}
                        </Draggable>
                      </div>
                    ) : !draggingPlayer ? <BenchSeat label={slot.kind === "GK" ? "YEDEK KL" : `Y${index + 1}`} /> : null}
                    <div className="pointer-events-none absolute inset-0 overflow-hidden opacity-0">{provided.placeholder}</div>
                  </div>
                )}
              </Droppable>
...
function DragFrame({ provided, snapshot, children }: { provided: DraggableProvided; snapshot: DraggableStateSnapshot; children: React.ReactNode }) {
  return <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} style={provided.draggableProps.style} className={["z-20 cursor-grab active:cursor-grabbing", snapshot.isDragging ? "opacity-0" : ""].join(" ")}>{children}</div>;
}

function DragClone({ provided, snapshot, children }: { provided: DraggableProvided; snapshot: DraggableStateSnapshot; children: React.ReactNode }) {
  return <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} style={{ ...provided.draggableProps.style, zIndex: 999999, pointerEvents: "none" }} className={snapshot.isDragging ? "will-change-transform drop-shadow-[0_26px_34px_rgba(0,0,0,.62)]" : ""}>{children}</div>;
}
