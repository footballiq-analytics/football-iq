  ...
  return (
    <section className="relative mx-auto w-full max-w-[860px] overflow-visible rounded-[30px] border border-emerald-200/10 bg-[#06140f] p-2 shadow-[0_30px_90px_rgba(0,0,0,.46)] sm:p-3">
      ...
      <div className="relative overflow-hidden rounded-[24px] border border-emerald-100/20 bg-[linear-gradient(180deg,rgba(2,28,17,.12),rgba(0,12,7,.42)),repeating-linear-gradient(90deg,#0e6a3d_0_12.5%,#0b5c36_12.5%_25%)] shadow-[inset_0_0_90px_rgba(0,19,8,.58)]">
        ...
        <div className="absolute left-[5%] top-[4.8%] z-40">
          <label className="block rounded-2xl border border-white/10 bg-[#05231b]/78 p-2 shadow-[0_10px_28px_rgba(0,0,0,.28)] backdrop-blur-xl">
            <span className="block text-[7px] font-black tracking-[.14em] text-emerald-100/55">DİZİLİŞ</span>
            <select
              value={formationLabel}
              onChange={(event) => onFormationChange(event.target.value)}
              className="mt-1 h-8 min-w-[104px] rounded-lg border border-emerald-300/30 bg-[#071c18] px-2 text-[10px] font-black text-emerald-200 outline-none focus:border-emerald-300/70"
            >
        ...
        <div className="relative z-10 flex min-h-[760px] flex-col px-2 pb-[150px] pt-20 sm:px-4 sm:pb-[162px] lg:min-h-[820px] lg:px-6">
          <div className="mb-2 flex justify-end px-2">
            <span className="rounded-full border border-white/10 bg-black/20 px-2.5 py-1 text-[8px] font-bold text-white/55 backdrop-blur-md">11 ASİL + 4 YEDEK</span>
          </div>
          ...
          <div className="flex flex-1 flex-col justify-evenly gap-3 py-1">
            {lineOrder.map((position) => {
              ...
              return (
                <motion.div layout key={position} className="grid w-full items-center justify-items-center gap-1 sm:gap-3" style={{ gridTemplateColumns: `repeat(${lineSlots.length}, minmax(0, 1fr))` }}>
                  {lineSlots.map((slot) => {
                            ...
                            {slot.player ? (
                              <Draggable draggableId={`player:${slot.player.id}`} index={0}>
                                {(dragProvided, dragSnapshot) => (
                                  <div ref={dragProvided.innerRef} {...dragProvided.draggableProps} {...dragProvided.dragHandleProps} className="cursor-grab active:cursor-grabbing">
                                    <PlayerCard
                                      compact
                                      player={slot.player!}
        ...
        <div className="absolute inset-x-[3.2%] bottom-[3.2%] z-20 rounded-2xl border border-emerald-100/15 bg-[linear-gradient(180deg,rgba(7,31,24,.58),rgba(2,17,14,.76))] p-2.5 shadow-[0_18px_34px_rgba(0,0,0,.28),inset_0_1px_0_rgba(255,255,255,.05)] backdrop-blur-xl sm:p-3">
          <div className="mb-2 flex items-center justify-between gap-3 px-1">
            <div><span className="text-[8px] font-black tracking-[.16em] text-emerald-300">YEDEK KULÜBESİ</span><p className="mt-0.5 text-[8px] font-medium text-white/45">Sahanın içine entegre 4 yedek oyuncu</p></div>
            <span className="rounded-full border border-white/10 bg-black/20 px-2 py-1 text-[8px] font-black text-white/55">{benchSlots.filter((slot) => slot.player).length}/4</span>
          </div>
                ...
                <Droppable key={slot.id} droppableId={slot.id} isDropDisabled={!valid}>
                  {(provided, snapshot) => (
                    <motion.div
                      ...
                      layout
                      className={[
                        "grid min-h-[110px] min-w-0 place-items-center rounded-2xl transition",
                    ...
                    >
                      {slot.player ? (
                        ...
                        <div className="grid h-[106px] w-full max-w-[82px] place-items-center rounded-2xl border border-dashed border-emerald-200/20 bg-black/10 px-2 text-center text-[7px] font-black tracking-[.08em] text-emerald-100/40">{slot.kind === "GK" ? "YEDEK GK" : `YEDEK ${index + 1}`}</div>
                      )}
                      {provided.placeholder}
                    </motion.div>
                  )}
...
function EmptySlot({ position, active, invalid }: { position: PlayerPosition; active: boolean; invalid: boolean }) {
  return (
    ...
    >
      <div><div className="mx-auto mb-2 h-10 w-8 rounded-[50%_50%_40%_40%] border border-emerald-100/16 bg-[linear-gradient(180deg,rgba(133,255,190,.08),rgba(0,0,0,.08))]" /><span className="block text-[6.5px] font-black tracking-[.1em] text-emerald-100/45">{lineLabel[position]}</span><small className="mt-0.5 block text-[6px] font-medium text-white/25">Buraya bırak</small></div>
    </motion.div>
  );
}
